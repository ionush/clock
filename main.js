...
const templatingEngine = (store, children) => {
  const replace = childBranch => {
    ...
    const found = Object.keys(store).find(e => e === id)
    if (found && store[id] !== childBranch.innerHTML) childBranch.innerHTML = store[id]
  }
  for (let i = 0; i < children.length; i++) {
    const { length } = children[i].children
    if (length === 0) replace(children[i])
    else if (length > 0) {
      for (let j = 0; j < length; j++) {
        ...
        } else replace(children[i].children[j])
      }
    }
  }
}
setInterval(clock, 1000)
setInterval(() => templatingEngine(time, children), 1000)
